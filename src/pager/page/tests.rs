...
use super::Page;
use crate::{file_header::PageSize, pager::ValidPage};
...
//   // let valid_pagesize = Box::new(page) as Box<dyn ValidPage>;

//   let maybe_valid_pagesize =
//     Page::<BUF_SIZE>::from_dyn(page_any).map(|data| -> Box<dyn ValidPage> { Box::new(data) });
